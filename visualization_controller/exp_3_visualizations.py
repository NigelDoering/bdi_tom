...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--summary",
        type=Path,
        default=Path("data/simulation_data/run_8/visualizations/exp_3/exp_3_summary.json"),
...
def plot_phase_boxplots(
    ...
    # Skip if no fractions or raw values unavailable
    if not fractions:
        return
    first_entry = results[0] if results else None
    if not first_entry or "values" not in first_entry[phase_key].get(str(fractions[0]), {}):
        return
...
def plot_entropy_histogram(output_path: Path, results: Sequence[Mapping[str, object]]) -> None:
    # Skip if no raw values available
    if not results:
        return
    first_frac = list(results[0]["post_entropy"].values())[0]
    if "values" not in first_frac:
        return
